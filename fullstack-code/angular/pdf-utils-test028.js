...
function generatePDF(data, outputPath, callback) {
  ...
  data.elements.forEach((element, index) => {
    ...
    if (displayCode) {
      ...
      console.log('00000000001:' + codelanguage);
      ...
      if (codelanguage === 'json') {
        ...
        lines.forEach(line => {
          if (line.trim() === '') {
            ...
            const keyMatch = line.match(/^(\s*)"([^"]+)":\s*(.*)$/); // Détecte "clé": valeur
            const bracketMatch = line.match(/^\s*[{}\[\],]\s*$/); // Détecte { } [ ] ,
      ...
      if (!codelanguageFound) {
        doc.fillColor('black').text(`${content}`, marginX, doc.y, {
          width,
          align: 'left',
          lineGap: 3,
        });
